...
for i in $(seq 1 50)
do
	sudo docker exec -d 50_customers_client_$i python3 mysimbdp-dataingest.py
done
sleep 500
for i in $(seq 1 50)
do
	sudo docker cp 50_customers_client_$i:response_time.txt response_time_$i.txt
	sudo docker cp 50_customers_client_$i:errors.txt errors_$i.txt
done
