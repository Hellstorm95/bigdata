...
for i in $(seq 1 100)
do
	sudo docker exec -d 100_customers_client_$i python3 mysimbdp-dataingest.py
done
sleep 1000
for i in $(seq 1 100)
do
	sudo docker cp 100_customers_client_$i:response_time.txt response_time_$i.txt
	sudo docker cp 100_customers_client_$i:errors.txt errors_$i.txt
done
