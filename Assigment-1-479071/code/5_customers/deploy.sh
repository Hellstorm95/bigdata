...
for i in $(seq 1 5)
do
	sudo docker exec -d 5_customers_client_$i python3 mysimbdp-dataingest.py
done
sleep 60
for i in $(seq 1 5)
do
	sudo docker cp 5_customers_client_$i:response_time.txt response_time_$i.txt
	sudo docker cp 5_customers_client_$i:errors.txt errors_$i.txt
done
